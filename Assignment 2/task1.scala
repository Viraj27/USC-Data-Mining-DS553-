...
def find_frequent_itemsets(baskets: Array[Set[String]], support: Int, totalBaskets: Long): Iterator[Set[String]] = {
  ...
  def task1_son(switch_case: Int, support: Int, input_file : String, output_file: String, sc : SparkContext) : Unit = {
    ...
    var baskets = input_data_rdd.map(l => (l(1), l(0)))
    if (switch_case == 1) {
      baskets = input_data_rdd.map(l => (l(0), l(1)))
    }
  ...
  def main(args: Array[String]): Unit = {
    val conf = new SparkConf().setAppName("DM_assign2_task1").setMaster("local[*]")
    val sc   = new SparkContext(conf)
    sc.setLogLevel("ERROR")

    val switch_case    = args(0).toInt
    val support        = args(1).toInt
    val input_file     = args(2).toString
    val output_file    = args(3).toString
    val startTime      = System.nanoTime()
    task1_son(switch_case, support, input_file, output_file, sc)
